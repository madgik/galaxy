...
4. You can also change the galaxy image that is going to be deployed, by changing the tag of the image (hbpmip/galaxy:v1.3.0).

Use the following command after changing the appropriate variables:
```
docker run -d -e EXAREME_IP=88.197.53.100 -e EXAREME_PORT=9090 -p 8090:80 hbpmip/galaxy:v1.3.0 /bin/bash -c "htpasswd -bc /etc/apache2/htpasswd admin password && ./createExaremeVariables.sh && /etc/init.d/apache2 restart && ./run.sh"
```
...
2. Create the docker image by running the command :
	```docker build -t hbpmip/galaxy:v1.3.0 . ```

3. (Optional) Pre-run the galaxy installer:
	- Run and access the image internally with ```docker run -i -t hbpmip/galaxy:v1.3.0 bash```
	...
	- Save the changes to the docker image with ```docker commit aaa527d9d22f hbpmip/galaxy:v1.3.0```
	- Upload the image ```docker push hbpmip/galaxy:v1.3.0```
